...
class Projectile {
public:
    inline static Texture2D bulletTexture = { 0 };

    static void InitTextures() {
        bulletTexture = LoadTexture("bullet.png");
        GenTextureMipmaps(&bulletTexture);
        SetTextureFilter(bulletTexture, TEXTURE_FILTER_TRILINEAR);
    }
	...
	void Draw() const {
        if (type == WeaponType::BULLET) {
            ...
            Vector2 origin = { texture.width * scale * 0.5f, texture.height * scale * 0.5f };
